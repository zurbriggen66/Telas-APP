...
import { CreditCard, Landmark, ArrowLeft, Check, User, MapPin, Package } from 'lucide-react';
...
const CheckoutSelection = () => {
    ...
    // --- FUNCIÓN PARA COTIZAR (ACTUALIZADA CON IDs ÚNICOS) ---
    const handleCotizarEnCheckout = async () => {
        ...
        try {
            ...
            if (data.error) {
                ...
                // 👇 MAGIA: Creamos un ID único combinando datos e índice 👇
                const opcionesConIdsUnicos = data.opciones.map((opcion, index) => ({
                    ...opcion,
                    // Generamos un ID como "Correo Argentino-Domicilio-0"
    ...
    const handleProcesarPago = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                // ...
    // Usamos el operador lógico OR (||) para capturar lo que venga, 
    // y si no viene nada, enviamos un valor por defecto para que no sea null
                        ...
                        <div className="checkout-form-section">
                            ...
                            <div className="opciones-entrega" style={{ display: 'flex', gap: '25px', marginBottom: '20px', fontFamily: "'Montserrat', sans-serif", fontSize: '0.95rem' }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                    <input type="radio" value="envio" checked={metodoEntrega === 'envio'} onChange={(e) => setMetodoEntrega(e.target.value)} style={{ accentColor: '#1A1A1A' }} />
                                    🚚 Envío a domicilio / Sucursal
                                </label>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                    <input type="radio" value="retiro" checked={metodoEntrega === 'retiro'} onChange={(e) => setMetodoEntrega(e.target.value)} style={{ accentColor: '#1A1A1A' }} />
                                    🏪 Retira en el local
                                </label>
                            </div>
                            ...
                            {metodoEntrega === 'envio' && (
                                <div className="datos-envio-container" style={{ animation: 'fadeIn 0.3s ease-out' }}>
                                    ...
                                    {/* GRILLA DE OPCIONES DE ENVÍO CON ESTILOS DE SELECCIÓN MEJORADOS */}
                                    {opcionesEnvio.length > 0 && (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '20px' }}>
                                            ...
                                            {opcionesEnvio.map((opcion) => {
                                                // 👇 Comparamos usando el nuevo id_unico 👇
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        style={{
                                                            padding: '15px 20px', 
                                                            // 👇 EFECTO VISUAL DE RECUADRO MARCADO 👇
                                                            border: estaSeleccionada ? '2px solid #1A1A1A' : '1px solid #E5E5E5',
                                                            boxShadow: estaSeleccionada ? '0 4px 6px rgba(0,0,0,0.05)' : 'none',
                                                            backgroundColor: estaSeleccionada ? '#FAFAFA' : 'white',
                                                            // ----------------------------------------
                                                            borderRadius: '8px', 
                                                            cursor: 'pointer', 
                                                            display: 'flex', 
                                                    ...
                                                    >
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                            <Package size={20} color={estaSeleccionada ? '#1A1A1A' : '#888'} />
                                                            <div>
                                                                <strong style={{ display: 'block', fontSize: '0.95rem', color: '#1A1A1A' }}>{opcion.proveedor}</strong>
                                                                <span style={{ fontSize: '0.8rem', color: '#666' }}>{opcion.servicio} • {opcion.tiempo_entrega}</span>
                                                            </div>
                                                        </div>
                                                        
                                                        {/* LADO DERECHO: PRECIO Y CÍRCULO DE CHECK */}
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                            <div style={{ fontWeight: '600', color: '#1A1A1A', fontSize: '1rem' }}>
                                                                ${opcion.costo.toLocaleString('es-AR')}
                                                            </div>
                                                            
                                                            {/* 👇 CÍRCULO INDICADOR VISUAL (IGUAL AL PAGO) 👇 */}
                                                            <div style={{
                                                                width: '22px', height: '22px', borderRadius: '50%',
                                                                border: estaSeleccionada ? '2px solid #1A1A1A' : '2px solid #E5E5E5',
                                                                backgroundColor: estaSeleccionada ? '#1A1A1A' : 'white',
